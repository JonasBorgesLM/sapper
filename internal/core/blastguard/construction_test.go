...
func TestNewProductionRequiresApproval(t *testing.T) {
	if _, err := New(model.TierProduction, testCaps); err == nil {
...
func TestNewProductionRefusedWhenNonInteractive(t *testing.T) {
	...
	if _, err := New(model.TierProduction, testCaps, WithProductionApproval(nil)); err == nil {
...
func TestNewProductionRefusedWhenConfirmationDeclined(t *testing.T) {
	...
	if _, err := New(model.TierProduction, testCaps, WithProductionApproval(declined)); err == nil {
...
func TestNewProductionAllowedWhenConfirmed(t *testing.T) {
	...
	g, err := New(model.TierProduction, testCaps, WithProductionApproval(confirmed))
	if err != nil {
