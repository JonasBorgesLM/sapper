...
// renderReport produces the self-contained HTML for a result. Values are
// escaped by html/template, and there are no external resources (no scripts,
// fonts or stylesheets fetched), so the report opens offline (FR-05).
//
// It shows the N-run aggregate (percentiles + spread) and the verdict. A
// per-window timeline is a future enhancement: it needs the collector to bucket
// samples by time, which v1 does not do — so the report does not fake one.
func renderReport(r model.Result) (string, error) {
	var buf bytes.Buffer
	if err := reportTmpl.Execute(&buf, r); err != nil {
